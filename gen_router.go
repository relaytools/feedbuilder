...
func greedySelectAndAssignN(relayAuthors map[string][]string, replicas int) ([]string, map[string][]string) {
    ...
    for _, authors := range relayAuthors {
        for _, a := range authors { if need[a] == 0 { need[a] = replicas } }
    }
    selected := []string{}
    ...
    for {
        ...
        for _, v := range need { if v > 0 { done = false; break } }
        if done { break }
        ...
        if bestGain == 0 || bestRelay == "" { break }

        // assign as many needing authors as possible to bestRelay
        for _, a := range relayAuthors[bestRelay] {
            if need[a] <= 0 { continue }
            if assignedSet[bestRelay] == nil { assignedSet[bestRelay] = make(map[string]struct{}) }
            if _, has := assignedSet[bestRelay][a]; has { continue }
    ...
    for r := range assigned { assigned[r] = uniqueSorted(assigned[r]) }
    for i := range selected { selected[i] = normalizeURL(selected[i]) }
...
func genRouterCmd(args []string) {
    ...
    relayAuthors := make(map[string][]string)
    {
        ...
        for _, line := range pairs {
            ...
            if _, ok := followsSet[pk]; !ok { continue }
    ...
    if *includeUnassigned {
        ...
        for _, s := range streams { for _, a := range s.Authors { counts[a]++ } }
        var needMore []string
        for a := range followsSet {
            if counts[a] < *replicas { needMore = append(needMore, a) }
        }
...
func readLinesMust(path string) []string {
    ...
    if err != nil { fmt.Fprintf(os.Stderr, "error reading %s: %v\n", path, err); os.Exit(1) }
    for i := range lines { lines[i] = normalizeURL(lines[i]) }
    return lines
}
...
func writeRouterConfig(path string, streams []streamConfig) error {
	...
	for _, s := range streams {
		fmt.Fprintf(w, "  %s {\n", s.Name)
		fmt.Fprintf(w, "    dir = \"%s\"\n", s.Dir)
		if s.Dir == "down" && len(s.Authors) > 0 {
			filter := map[string]any{"authors": s.Authors}
			if s.Kinds != "" {
				var kinds any
